...
8.  **MCP 服务** 🆕：内置 Model Context Protocol (MCP) 服务，在 localhost:23337 自动暴露 JSON-RPC 接口，允许外部应用通过标准协议访问 Zotero 数据。支持获取 PDF 内容、AI 总结，以及管理笔记的增删改查。详见 [MCP 服务文档](./docs/MCP_SERVICE.md)。
